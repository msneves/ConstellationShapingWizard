...
def plot_filters():
    ...
    st.session_state.ax_filters[0].stem(np.arange(st.session_state.ntaps_ps)-st.session_state.ntaps_ps//2,st.session_state.taps_ps[:,0,0]/tf.reduce_max(st.session_state.taps_ps[:,0,0]), 'b', markerfmt='bo',label='PS,I')
    st.session_state.ax_filters[0].stem(np.arange(st.session_state.ntaps_ps)-st.session_state.ntaps_ps//2,st.session_state.taps_ps[:,1,1]/tf.reduce_max(st.session_state.taps_ps[:,1,1]), 'g', markerfmt='go',label='PS,Q')
    st.session_state.ax_filters[0].stem(np.arange(st.session_state.ntaps_mf)-st.session_state.ntaps_mf//2,st.session_state.taps_mf[:,0,0]/tf.reduce_max(st.session_state.taps_mf[:,0,0]), 'y', markerfmt='yo',label='MF,I')
    st.session_state.ax_filters[0].stem(np.arange(st.session_state.ntaps_mf)-st.session_state.ntaps_mf//2,st.session_state.taps_mf[:,1,1]/tf.reduce_max(st.session_state.taps_mf[:,1,1]), 'r', markerfmt='ro',label='MF,Q')
    ...
    w, h = freqz(st.session_state.taps_bw[:,0,0], worN=100, fs=2) 
    ...
    w, h = freqz(st.session_state.taps_ps[:,0,0], worN=100, fs=2) 
    frequency_response = 20 * np.log10(abs(h))
    frequency_response -= max(frequency_response)
    st.session_state.ax_filters[1].plot(w, frequency_response, 'b',label='PS,I')
    w, h = freqz(st.session_state.taps_ps[:,1,1], worN=100, fs=2) 
    frequency_response = 20 * np.log10(abs(h))
    frequency_response -= max(frequency_response)
    st.session_state.ax_filters[1].plot(w, frequency_response, 'g',label='PS,Q')
    w, h = freqz(st.session_state.taps_mf[:,0,0], worN=100, fs=2) 
    frequency_response = 20 * np.log10(abs(h))
    frequency_response -= max(frequency_response)
    st.session_state.ax_filters[1].plot(w, frequency_response, 'y',label='MF,I')
    w, h = freqz(st.session_state.taps_mf[:,1,1], worN=100, fs=2) 
    frequency_response = 20 * np.log10(abs(h))
    frequency_response -= max(frequency_response)
    st.session_state.ax_filters[1].plot(w, frequency_response, 'r',label='MF,Q')
    st.session_state.ax_filters[1].set_ylim(-20,0)
